...
> Step4. 브라우저는 렌더 트리를 기반으로 각 요소의 위치(x, y), 크기(width, height)를 계산해 실제 화면 배치를 위한 레이아웃 정보를 결정한다. 이 과정을 레이아웃 또는 리플로우(Reflow)라고 한다.

- 렌더 트리(Render Tree)가 생성되면, 브라우저는 각 노드가 화면의 **어디에 배치될지**, **얼마나 큰 영역을 차지할지** 계산해야 한다.
- 이때 브라우저는 렌더 트리를 루트에서부터 순차적으로 내려가며 다음 정보를 계산한다.
  - 요소의 크기(width, height)
  - 위치(top, left 등)
  - 마진, 패딩, 보더
  - 폰트 크기와 줄간격(line-height)
  - 화면(Viewport)의 크기
  - 부모 요소의 레이아웃 제약(Flex, Grid 등)
- 렌더 트리는 `보여줘야 하는 요소들`의 목록일 뿐이며, 이 요소들이 **화면에서 실제로 어디에 위치하는지**는 전혀 결정되어 있지 않다. 레이아웃 단계에서는 다음 두 가지를 계산하여 화면 배치를 완성한다.
  - **각 요소의 크기 계산 (Size Calculation)**
  - **각 요소의 위치 계산 (Position Calculation)**
- 레이아웃 과정의 주요 특징

  - 상향식 계산 (Top-down Calculation)
    - 렌더 트리의 루트(예: `<html>` → `<body>`)에서 출발하여 **부모의 크기와 위치가 계산되어야 자식이 계산될 수 있다.**
    - 예:  
      부모가 `width: 50%`인 경우  
      → 부모의 실제 px 값을 알아야  
      → 자식의 `%` 계산도 가능하다.
  - 렌더링 성능에 중요한 단계
    - 레이아웃은 비용이 큰 작업이다. 특히 다음 조건이 충족되면 **전체 문서를 다시 계산해야 할 수 있다.**
    - 예:
      - DOM 구조 변경 (요소 추가/삭제)
      - CSS 속성 변경 (`width`, `font-size`, `display`, `position` 등)
      - 뷰포트 변화 (브라우저 창 크기 변경)
      - 스크롤 위치 변화로 레이아웃이 달라지는 경우
    - 이런 재계산을 `Reflow(리플로우)`라고 한다.
  - 레이아웃이 다시 발생(Reflow)하는 CSS 속성들
    - 아래의 작업들은 페인트만 다시하는 속성들보다 훨씬 비용이 크다.
      - width / height
  ...
  - Reflow vs Repaint
    - 렌더링 과정에서 자주 헷갈리는 개념
      | 개념 | 설명 |
      |------|------|
      | **Reflow (레이아웃)** | 요소의 크기나 위치가 다시 계산됨. 가장 비용이 큰 작업. |
      | **Repaint (페인트)** | 레이아웃은 같지만 색상, 배경 등만 변경되어 다시 그리는 경우. 상대적으로 비용 낮음. |
    - 예:
      - `color` 변경 → **Repaint만 발생**
      - `width`, `font-size` 변경 → **Reflow + Repaint 발생**

- 레이아웃 단계 요약
  - Render Tree → 부모부터 자식 순서로 크기·위치 계산 → Reflow(필요 시) → 다음 단계: 페인팅(Paint)
  - 레이아웃은 실제 화면의 배치를 결정하는 아주 중요한 과정이며, 잘못된 DOM 조작이나 스타일 변경은 Reflow를 빈번하게 발생시켜 성능 저하를 일으킬 수 있다.
...
> Step5. 브라우저는 레이아웃 단계에서 계산된 정보를 바탕으로 각 요소를 실제 픽셀로 화면에 그린다. 이 과정을 페인팅(Paint)이라고 한다.

- 레이아웃(Layout) 단계까지 완료되면, 브라우저는 각 요소의 **위치와 크기 정보**를 모두 알고 있는 상태가 된다. 하지만 이 시점까지도 화면에는 아직 아무것도 그려지지 않았으며, 실제로 사용자가 보는 화면을 만들기 위해서는 **픽셀 단위로 요소를 그리는 과정**이 필요하다. 이 과정을 `페인팅(Paint)`이라고 한다.
- 페인팅 단계에서 수행되는 작업
  - 페인팅 단계에서는 다음과 같은 시각적 요소들이 처리된다.
    - 텍스트 색상(color)
    ...
    - 이미지(img)
    - 글꼴(font), 글자 렌더링
- 즉, **어디에 무엇을 그릴지**가 아닌, `그 자리에 어떤 픽셀을 칠할지`를 결정하는 단계이다.
- 레이어(Layer) 개념
  - 브라우저는 모든 요소를 한 번에 그리지 않고, 여러 개의 **레이어(layer)** 로 나누어 페인팅을 수행한다.
  - 일부 요소는 별도의 레이어로 분리된다.
    - 대표적인 예:
      - `position: fixed`
      - `transform`
      - `opacity`
      - `will-change`
      - CSS 애니메이션이 적용된 요소
  - 이렇게 레이어를 분리하면, **일부 요소만 변경될 때 전체 화면을 다시 그리지 않아도 되는 장점**이 있다.
- Repaint (리페인트)
  - 레이아웃은 변하지 않고 **시각적 속성만 변경**되었을 때 발생하는 다시 그리기 작업을 `Repaint`라고 한다.
  - Repaint는 Reflow(Layout)보다는 비용이 적지만, 빈번하게 발생하면 성능에 영향을 줄 수 있다.
- 페인팅 단계 요약
  - 페인팅은 실제로 화면에 그리는 단계
  - 레이아웃보다 비용은 적지만, 잦은 Repaint 역시 성능 저하의 원인이 될 수 있다.
  - 이후 단계에서 레이어를 합성하는 컴포지팅(Composite) 과정이 이어진다.
...
> Step6. 브라우저는 페인팅(Paint)이 끝난 여러 레이어를 합성하여 최종 화면을 만든다. 이 과정을 컴포지팅(Composite)이라고 한다.

- 페인팅 단계까지 완료되면, 브라우저는 각 요소를 **여러 개의 레이어(layer)** 단위로 그려놓은 상태가 된다.
- 컴포지팅 단계에서는 이 레이어들을 **올바른 순서와 위치로 합성**하여, 사용자가 실제로 보게 되는 최종 화면을 만든다.
- 이 과정은 주로 **GPU(Graphics Processing Unit)** 에서 처리되어 성능상 이점을 가진다.
- 컴포지팅 단계에서 수행되는 작업
  - 컴포지팅 단계에서는 다음과 같은 작업이 이루어진다.
    - 레이어의 **z-index 순서 결정**
    - 레이어의 **위치 이동**
    - **투명도(opacity) 적용**
    - **transform(translate, scale, rotate 등) 적용**
    - 여러 레이어를 하나의 화면으로 합성
- 중요한 점은 `컴포지팅은 픽셀을 다시 그리지 않는다`는 것이다.
- Composite-only 작업 (가장 빠른 렌더링)
  - 일부 CSS 속성 변경은 **레이아웃도, 페인팅도 발생시키지 않고** 오직 컴포지팅 단계만 다시 수행한다.
    - 대표적인 속성:
      - `transform`
      - `opacity`
  - 예를 들어, transform 속성은 Reflow와 Repaint를 발생시키지 않고, Composite만 발생한다. 이는 GPU에서 레어이 위치만 이동하기 때문에 매우 빠른 애니메이션이 가능하다.
- 컴포지팅 단계 요약
  - 컴포지팅은 렌더링 파이프라인의 마지막 단계
  - 레이아웃과 페인팅에 비해 비용이 매우 적다.
  - GPU 가속을 활용해 부드러운 애니메이션과 스크롤을 가능하게 한다.
...
#### 8. CRP(Critical Rendering Path)와 브라우저 렌더링의 관계

---

> CRP(Critical Rendering Path)는 브라우저가 HTML을 다운로드한 순간부터,  
> **화면에 첫 픽셀을 그리기까지(FCP/LCP)** 반드시 거쳐야 하는 렌더링 경로를 의미한다.

- 즉, **CRP는 브라우저 렌더링 과정을 "성능 관점에서 잘라낸 핵심 경로"** 이다.
- 브라우저 렌더링과 CRP의 차이
  | 구분 | 브라우저 렌더링 | CRP |
  |-----|----------------|-----|
  | 관점 | 동작 원리 중심 | 성능 최적화 중심 |
  | 범위 | 전체 렌더링 파이프라인 | 초기 화면에 필요한 최소 경로 |
  | 목적 | “어떻게 화면을 그리는가” | “언제 화면이 보이는가” |
- 브라우저 렌더링은 전체 과정을 설명하고, CRP는 그중에서도 **사용자가 화면을 보기까지 반드시 필요한 단계만 추린 것**이다.
- CRP는 어디까지를 의미하는가?
  - CRP는 다음 질문에 답하기 위한 개념이다.
  - > “이 리소스가 없으면 **첫 화면을 그릴 수 있는가?**”
  - 그래서 CRP에는 **초기 렌더링을 막는 리소스들**만 포함된다.
- CRP에 포함되는 핵심 단계는 다음과 같다.
  - HTML 다운로드 & 파싱
  - → CSS 다운로드 & 파싱 (Render-blocking)
  - → Render Tree 생성
  - → Layout
  - → Paint (첫 화면)
- JavaScript, 이미지, 폰트 등은 **CRP에 포함될 수도 있고 아닐 수도 있다.** 핵심 기준은 `초기 렌더링을 막는가?` 이다.
- CRP 관점에서 중요한 리소스들
  - Render-blocking CSS
    - CSS는 **렌더링 차단 리소스**
    - CSSOM이 완성되기 전에는 Render Tree를 만들 수 없다.
      - **CRP에 항상 포함된다.**
  - Render-blocking JavaScript
    - `<script>`는 DOM 파싱을 멈추고 실행된다.
    - JS는 DOM/CSSOM을 변경할 수 있으므로 순서 보장이 필요하다.
      - **CRP를 지연시킬 수 있다.**
  - 이미지 / 폰트
    - 보통 CRP에 포함되지 않음
    - 단, **LCP 요소**이거나 **웹 폰트**는 CRP에 포함될 수 있음
- CRP와 성능 지표의 연결
  - CRP는 다음 성능 지표와 직접적으로 연결된다.
    | 지표 | 의미 |
    |-----|-----|
    | **FCP** | 첫 텍스트/이미지가 화면에 보이는 시점 |
    | **LCP** | 가장 큰 콘텐츠가 보이는 시점 |
    | **TTI** | 사용자 인터랙션 가능 시점 |
  - CRP를 줄이면 FCP / LCP가 빨라진다.
- CRP 최적화가 곧 렌더링 최적화
  - CRP를 줄이기 위한 대표적인 방법
    - CSS 최소화 및 분리 (Critical CSS)
    - `<script defer>` / `<script async>` 사용
    - 불필요한 JS 제거
    - 코드 스플리팅
    - 초기 렌더링에 필요 없는 리소스 지연 로딩
    - 폰트 preload / font-display 설정
